...
class SeoController {
    async add(req: Request, res: Response) {
        try {
            const { metaTitle, metaDescription, position } = req.body;
            ...
            const created = await SeoService.add({metaTitle, metaDescription, position});
    ...
    async all(req: Request, res: Response) {
        try {
            ...
            let data: any = await SeoService.allByPosition(position);
            if (page && size) {
    ...
    async single(req: Request, res: Response) {
        try {
            ...
            const data: any = await SeoService.singleByField({ customID });
            if (!data) {
                return res
                    .status(OK)
    ...
    async edit(req: Request, res: Response) {
        try {
            ...
            const updated = await SeoService.update({ customID }, { metaTitle, metaDescription, position  });
            if (updated)
                return res
                    .status(OK)
                    .send(success(ErrorMessage.HTTP_OK, req.body));
    ...
    async delete(req: Request, res: Response) {
        try {
            const { customID } = req.params;
            const deleted = await SeoService.destroy({ customID });
            if (deleted)
                return res
                    .status(OK)
                    .send(success(ErrorMessage.HTTP_OK, req.body));
